...
std::optional<RunnerArgs> parse_args(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--expected-runtime-library-sha256" && index + 1 < argc) {
        ...
        if (argument == "--software-key-source" && index + 1 < argc) {
            const std::string source = argv[++index];
            if (source == "none") {
        ...
        if (argument == "--software-key-env-var" && index + 1 < argc) {
            args.software_key_env_var = argv[++index];
            continue;
        }
        if (argument == "--software-key-file" && index + 1 < argc) {

...
All public V2 structures have `struct_size`, `abi_version`, explicit reserved
space, fixed-width scalar fields, fixed-capacity text, and exported native
`sizeof`/`alignof` functions. Provenance records retain stream, table,
...
stop/destroy, without wiring V2 into AlorEngine trading logic.
...
wrong-session refusals, receipt-write failure, the fake

...
Phase 5B does not add live order sending, TEST order sending, CGate publisher lifecycle, `cg_pub_*` usage, router submission, write-side C ABI, AlorEngine order-entry integration, production profiles, public market data, persistence, or evidence artifacts.
...
All command families have an offline representation and structural codec coverage. Business semantics, publisher interaction, router callbacks, fake transaction outcomes, and replication confirmation flows remain deferred.
...
The tests cover command coverage, required-field failures, invalid enum failures, invalid price/quantity failures, fixed-string length rejection, deterministic golden encoding, reply/error decoding, generated metadata determinism, and no-send guards.

## Known Limitations

The byte layout is an offline locked-layout model derived from Phase 5A field order and field type tokens. Live CGate publisher integration may require an additional adapter step once official publisher runtime behavior is introduced.

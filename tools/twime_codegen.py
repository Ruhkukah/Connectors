    ...
    for item in parsed["enums"]:
        if item["protocol_item_id"].startswith("twime_enum_"):
            ...
            for value in item["values"]:
                ...
                lines.append(
                    f"inline constexpr {base_type} {value['name']} = "
                    f"static_cast<{base_type}>(1u << {bit_index});"
                )
            lines.append(f"}}  // namespace {item['enum_name']}Mask")
            sets.append("\n".join(lines))

    sections.extend(section for section in enums + sets if section)
    sections.extend(["", "}  // namespace moex::twime_sbe", ""])
    return "\n".join(sections)
...
def _generated_messages(parsed: dict) -> str:
    ...
    lines.extend(f"    {item['message_name']} = {item['template_id']}," for item in parsed["messages"])
    lines.extend(["};", "", "}  // namespace moex::twime_sbe", ""])
    return "\n".join(lines)

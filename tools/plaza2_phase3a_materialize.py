...
MESSAGE_SURFACES = [
    ...
    {
        "protocol_item_id": "plaza2_message_cg_msg_p2repl_replstate",
        "artifact_id": "cgate_docs",
        "surface_kind": "replication_resume_state_notification",
        "message_name": "CG_MSG_P2REPL_REPLSTATE",
        "source_reference": "CGate listener callback surface and repl_resume example",
        "first_phase": "phase3d_fake_engine",
        "notes": (
            "Resume-state callback surface used to persist and restore replication "
            "position across reconnects and restart-safe resumes."
        ),
    },
    {
        "protocol_item_id": "plaza2_message_cg_msg_tn_begin",
        "artifact_id": "cgate_docs",
        "surface_kind": "transaction_boundary_begin",
        "message_name": "CG_MSG_TN_BEGIN",
        "source_reference": "CGate listener callback transactional boundaries",
        "first_phase": "phase3d_fake_engine",
        "notes": (
            "Begin marker for a replicated transactional block; downstream state "
            "must not be published as committed before the matching commit."
        ),
    },
    {
        "protocol_item_id": "plaza2_message_cg_msg_tn_commit",
        "artifact_id": "cgate_docs",
        "surface_kind": "transaction_boundary_commit",
        "message_name": "CG_MSG_TN_COMMIT",
        "source_reference": "CGate listener callback transactional boundaries and repl_resume example",
        "first_phase": "phase3d_fake_engine",
        "notes": (
            "Commit marker for a replicated transactional block and the stable "
            "restart/resume checkpoint referenced by repl_resume."
        ),
    },
    {
...
def build_runtime_expectations(environment: str) -> list[dict]:
    ...
    scheme_stream_names = sorted(
        PRIVATE_CORE_STREAMS | PUBLIC_COMMON_STREAMS | PUBLIC_AGGR_STREAMS | PUBLIC_ORDLOG_STREAMS
    )
    scheme_stream_ids = [stream_protocol_item_id(name) for name in scheme_stream_names]
    expectations = []
...
def validate_outputs(project_root: Path, output_root: Path) -> None:
    ...
    required_message_ids = {
        ...
        "plaza2_message_cg_msg_p2repl_replstate",
        "plaza2_message_cg_msg_tn_begin",
        "plaza2_message_cg_msg_tn_commit",

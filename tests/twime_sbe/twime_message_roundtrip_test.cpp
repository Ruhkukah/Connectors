...
int main() {
    try {
        constexpr std::uint16_t required_template_ids[] = {
            5000, 5001, 5002, 5003, 5004, 5005, 5006, 5007, 5008, 5009, 6000, 6004, 6005, 6006,
            6007, 6008, 6009, 6010, 6011, 7007, 7010, 7014, 7015, 7016, 7017, 7018, 7019, 7020,
        };
        for (const auto template_id : required_template_ids) {
            moex::twime_sbe::test::require(moex::twime_sbe::TwimeSchemaView::find_message_by_template_id(template_id) !=
                                               nullptr,
        ...
        for (const auto& message : moex::twime_sbe::TwimeSchemaView::messages()) {
            ...
            moex::twime_sbe::test::require(computed_block_length == message.block_length,
        ...
        for (const auto& message : moex::twime_sbe::TwimeSchemaView::messages()) {
            ...
            moex::twime_sbe::test::require(encode_error == moex::twime_sbe::TwimeDecodeError::Ok,
            ...
            moex::twime_sbe::test::require(decode_error == moex::twime_sbe::TwimeDecodeError::Ok,
                                           "sample message decode failed");
            moex::twime_sbe::test::require(decoded.metadata != nullptr, "decoded metadata missing");
            moex::twime_sbe::test::require(decoded.metadata->template_id == message.template_id,
                                           "decoded template mismatch");
        }
        ...
        for (const auto& path : fs::directory_iterator(fixtures_dir)) {
            ...
            const auto expected_hex = moex::twime_sbe::test::read_text_file(fixtures_dir / "expected" /
                                                                            (path.path().stem().string() + ".hex"));
            const auto trimmed_hex = expected_hex.substr(0, expected_hex.find_first_of("\r\n"));
            moex::twime_sbe::test::require(moex::twime_sbe::test::bytes_to_hex(bytes) == trimmed_hex,
            ...
            moex::twime_sbe::test::require(moex::twime_sbe::test::fixture_matches_message(fixture, decoded),

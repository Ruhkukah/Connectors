...
int main() {
    try {
        ...
        auto config = moex::twime_trade::test::make_live_session_config(server.port(), "phase2f_live_reconnect_on");
        config.policy.reconnect_enabled = true;
        config.policy.max_reconnect_attempts = 2;
        ...
        moex::twime_sbe::test::require(runner.health_snapshot().state == TwimeSessionState::Faulted,
                                       "remote close should fault the session before reconnect");
        moex::twime_sbe::test::require(!runner.reconnect_due(), "reconnect must respect one-second backoff");

        runner_clock.advance(1000);
        (void)runner.poll_once();
        moex::twime_sbe::test::require(runner.session_metrics().reconnect_attempts >= 2,
                                       "reconnect-enabled runner must record a second reconnect attempt");

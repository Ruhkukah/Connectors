...
def render(root: Path) -> str:
    ...
    lines = [
        ...
        f"- Structure preserved: **{register['form_structure']['answer_count']} answer fields**, including **{register['form_structure']['printed_stream_checkbox_count']} printed stream checkboxes**.",
        f"- PR #66 baseline at review start: `{register['source_identity']['source_head_at_review_start']}`.",
        f"- CI run [{register['source_identity']['ci_run']}]({register['source_identity']['ci_url']}) at the same exact head: `connector-validation` and `component-sanitizers` both **SUCCESS**.",
        f"- Deployable Linux binary SHA-256: **not claimed**. {register['source_identity']['binary_note']}",
        f"- Historical receipt source prefix `{register['source_identity']['historical_receipt_source_prefix']}` is a different snapshot and is not promoted to this head.",
        "",
    ...
    lines += [
        ...
        "",
        f"{alias['finding']} {alias['disposition']}",
        "",
    ...
    for stream in register["stream_inventory"]:
        lines.append(
            "| `{service}` | `{policy}` | {supported} | {requested} | {consumed} | `{opened}` |".format(
                service=stream["service"],
                policy=stream["scheme_policy"],
                supported=md(", ".join(stream["supported_current_scheme_tables"])),
                requested=md(stream["requested_tables"]),
                consumed=md(", ".join(stream["consumed_tables"])),
                opened=stream["open_receipt"],
            )
        )
    ...
    lines += [
        ...
        "",
        f"`{historical['tables'][0]}` and `{historical['tables'][1]}` are **historical compatibility-only** for this draft. {historical['repository_lock_note']}",
        "",
    ...
    for field in register["answers"]:
        lines += [
            ...
            "",
            f"- Proposed answer: {field['answer']}",
        ...
        for ref in field["evidence_refs"]:
            ...
            lines += [
                f"- Source: {sources}",
                f"- Automated test/evidence: {evidence['automated_test']}",
                f"- Live evidence: {evidence['live_evidence']}",
            ]
        lines.append("")
    lines += [
